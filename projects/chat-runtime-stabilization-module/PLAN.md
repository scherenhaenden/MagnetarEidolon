...
| `task-chatfix-105` | `ms-chatfix-02` | Add runtime diagnostics and health endpoints | Core Team | 2 | 12 | in_review | Heartbeat-first direction chosen: add a lightweight `/api/heartbeat` liveness probe now, then layer provider-specific deep diagnostics on top so backend-down and provider-down states stay easy to separate. |
