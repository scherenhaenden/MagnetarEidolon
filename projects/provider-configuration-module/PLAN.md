...
| `task-provider-102` | `ms-provider-01` | Implement Angular provider configuration screen | Core Team | 4 | 31 | in_review | Added shell tab, editable provider controls, and the left-side quick-add/accordion navigation. |
...
| `task-provider-110` | `ms-provider-01` | Add CRUD and preset onboarding to the Providers UI | Core Team | 4 | 24 | in_progress | Preset-based provider creation, editable runtime fields, local persistence, removal flows, and accordion-style quick-add navigation are now in place; backend-backed persistence still remains. |
